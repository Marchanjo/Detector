...
public class DJIVideoStreamDecoder implements NativeHelper.NativeDataListener {
   ...
   // private static DJIVideoStreamDecoder instance;
    private  DJIVideoStreamDecoder instance;
    ...
    public Handler yuvListenerHandler;
   ...
   /* public void setYuvDataListener(DJICodecManager.YuvDataCallback yuvDataListener) {
    ...
    //private Handler YUVCallback;
    public void setCustomYuvDataListener(DJIVideoStreamDecoder.CustomYuvDataCallback yuvDataListener) {
        this.yuvDataListener = yuvDataListener;

    }

    public interface CustomYuvDataCallback {
        public void onYuvDataReceived(ByteBuffer yuvFrame, int dataSize, int width, int height);
      //  public void onYuvDataReceived(int dataSize, int width, int height, Image image);
    }

    //private DJICodecManager.YuvDataCallback yuvDataListener;
    private DJIVideoStreamDecoder.CustomYuvDataCallback yuvDataListener;
   ...
   public DJIVideoStreamDecoder() {
        ...
        Log.e(TAG,"started stream decoder thread");
    }


    public synchronized  DJIVideoStreamDecoder getInstance() {
        if (instance == null) {
     ...
     *                should set "null" surface when calling the "configure" method of MediaCodec.
     */
    public void init(Context context, Surface surface, SurfaceTexture texture) {
    ...
    private void initCodec() {
        ...
        if (codec != null) {
            releaseCodec();
            loge("CODEC RELEASED IN INIT CODEC");
        }
        ...
        if (surface == null) {
            loge("initVideoDecoder: yuv output");
            // The surface is null, which means that the yuv data is needed, so the color format should
            // be set to YUV420.
            format.setInteger(MediaFormat.KEY_COLOR_FORMAT, MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar);} else {
            loge("initVideoDecoder: display");
    ...
    private void startDataHandler() {
        ...
        Log.e(TAG,"started data handler thread");
    ...
    private void stopDataHandler() {
        ...
        if (Build.VERSION.SDK_INT >= 18) {
          boolean quit =   dataHandlerThread.quitSafely();
          dataHandlerThread.interrupt();
          if (quit)
            {Log.e(TAG,"destroyed data handler thread");}
            else{
              Log.e(TAG,"DID NOT DESTROY data handler thread");
          }
        } else {
        ...
        if (handlerThreadNew == null || handlerThreadNew.isAlive()) {
            return;
        }
        if (handlerNew != null) {
            handlerNew.removeCallbacksAndMessages(null);
        ...
        if (Build.VERSION.SDK_INT >= 18) {
           boolean quit =  handlerThreadNew.quitSafely();
           handlerThreadNew.interrupt();
           if (quit){
               Log.e(TAG,"destroyed stream decoder thread");
           }
           else{
               Log.e(TAG,"DID NOT DESTROY stream decoder thread");
           }
        } else {
            handlerThreadNew.quit();
        }

        try {
           handlerThreadNew.join(3000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    ...
    public void changeTexture(SurfaceTexture texture){
        ...
        if (inIndex >= 0) {
            ...
            if (outIndex >= 0) {
                ...
                if (surface == null && yuvDataListener != null) {
                   ...
                   // img = codec.getOutputImage(outIndex); // get image before buffer?
                    ByteBuffer yuvDataBuf = codec.getOutputBuffer(outIndex);

                    /*try{img = codec.getOutputImage(outIndex);}
                    catch(MediaCodec.CodecException exception){
                        loge("MediaCodec Exception for getOutputImage(outputIndex)");
                    }
                    catch(IllegalStateException exception){
                        loge("IllegalStateException for getOutputImage(outputIndex)");
                    }*/
                    ...
                    if (yuvDataListener != null) {
                        yuvDataListener.onYuvDataReceived(yuvDataBuf, bufferInfo.size - bufferInfo.offset,  width, height);
                            loge("YUV Listener NOT NULL");
                       // yuvDataListener.onYuvDataReceived(bufferInfo.size - bufferInfo.offset, width, height, img);
                    }
                ...
                codec.releaseOutputBuffer(outIndex, true);
                    ...
                    if (e!= null)
                        loge("MEDIA CODEC SHIT: " + e.getMessage());
                }
                ...
                loge("NEW FORMAT:" + codec.getOutputFormat());
            }
        } else {
            codec.flush();
        }
    ...
    public void stop() {
        ...
        if (dataHandler != null) {
            loge("dataHandler NOT NULL");
            dataHandler.removeCallbacksAndMessages(null);
        }


        if (frameQueue != null) {
            loge("frameQueue NOT NULL");
            frameQueue.clear();
            hasIFrameInQueue = false;
        }
        if (codec != null) {
